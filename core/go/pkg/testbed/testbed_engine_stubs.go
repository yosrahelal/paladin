...
func (tb *testbed) ExecTransactionSync(ctx context.Context, tx *pldapi.TransactionInput) (receipt *pldapi.TransactionReceipt, err error) {
	txm := tb.c.TxManager()
	var txIDs []uuid.UUID
	err = keymanager.DBTransactionWithKRC(ctx, tb.c.Persistence(), tb.c.KeyManager(), func(dbTX persistence.DBTX, kr components.KeyResolver) (postCommit func(), err error) {
