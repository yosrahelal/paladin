...
func (e *endorsementGatherer) GatherEndorsement(ctx context.Context, tx *components.PrivateTransaction, partyName string, endorsementRequest *prototk.AttestationRequest) (*prototk.AttestationResult, *string, error) {
	...
	// Invoke the domain
	endorseRes, err := e.psc.EndorseTransaction(ctx, tx, endorsementRequest, &prototk.ResolvedVerifier{
		Lookup:    partyName,
