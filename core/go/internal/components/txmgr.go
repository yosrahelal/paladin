...
type TXManager interface {
	...
	GetTransactionDependencies(ctx context.Context, id uuid.UUID) (*ptxapi.TransactionDependencies, error)
	GetPublicTransactionByNonce(ctx context.Context, from tktypes.EthAddress, nonce tktypes.HexUint64) (*ptxapi.PublicTxWithBinding, error)
	GetPublicTransactionByHash(ctx context.Context, hash tktypes.Bytes32) (*ptxapi.PublicTxWithBinding, error)
	QueryTransactions(ctx context.Context, jq *query.QueryJSON, pending bool) ([]*ptxapi.Transaction, error)
	QueryTransactionsFull(ctx context.Context, jq *query.QueryJSON, pending bool) (results []*ptxapi.TransactionFull, err error)
