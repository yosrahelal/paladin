...
type PublicTxManager interface {
	...
	UpdateTransaction(ctx context.Context, id uuid.UUID, pubTXID uint64, from *tktypes.EthAddress, tx *pldapi.TransactionInput, publicTxData []byte, txmgrDBUpdate func(dbTX persistence.DBTX) error) error
}
