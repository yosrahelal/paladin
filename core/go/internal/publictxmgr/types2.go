...
func (ctx *RunningStageContext) SetSubStatus(subStatus PubTxSubStatus) {
...
type StatusUpdater interface {
	UpdateSubStatus(ctx context.Context, tx *ptxapi.PublicTx, subStatus PubTxSubStatus, action BaseTxAction, info *fftypes.JSONAny, err *fftypes.JSONAny, actionOccurred *tktypes.Timestamp) error
}
...
func (sOut *RunningStageContextPersistenceOutput) UpdateSubStatus(action BaseTxAction, info *fftypes.JSONAny, err *fftypes.JSONAny) {
	...
	sOut.StatusUpdates = append(sOut.StatusUpdates, func(p StatusUpdater) error {
		return p.UpdateSubStatus(sOut.Ctx, sOut.InMemoryTx.GetTx(), sOut.SubStatus, action, info, err, &actionOccurred)
	})
}
...
type InFlightTransactionStateManager interface {
	...
	// stage management
	StartNewStageContext(ctx context.Context, stage InFlightTxStage, substatus PubTxSubStatus)
