...
func TestProduceLatestInFlightStageContextSigning(t *testing.T) {
	...
	mTS.On("AddSubStatusAction", mock.Anything, mtx.ID, components.BaseTxSubStatusReceived, components.BaseTxActionSign, fftypes.JSONAnyPtr(`{"hash":"`+txHash+`"}`), (*fftypes.JSONAny)(nil), mock.Anything).Return(nil).Maybe()
	...
	assert.NotNil(t, rsc.StageOutputsToBePersisted)
	assert.Equal(t, 1, len(rsc.StageOutputsToBePersisted.HistoryUpdates))
	mTS.On("AddSubStatusAction", mock.Anything, mtx.ID, components.BaseTxSubStatusReceived, components.BaseTxActionSign, (*fftypes.JSONAny)(nil), fftypes.JSONAnyPtr(`{"error":"sign error"}`), mock.Anything).Return(nil).Maybe()
	...
	mTS.On("AddSubStatusAction", mock.Anything, mtx.ID, components.BaseTxSubStatusReceived, components.BaseTxActionSign, mock.Anything, mock.Anything, mock.Anything).Return(nil).Maybe()
	tOut = it.ProduceLatestInFlightStageContext(ctx, &baseTypes.OrchestratorContext{
		AvailableToSpend:         nil,
		PreviousNonceCostUnknown: false,
	})
