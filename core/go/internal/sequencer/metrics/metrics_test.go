...
func TestInitMetrics(t *testing.T) {
	...
	assert.Equal(t, metricFamilies[0].GetName(), "distributed_sequencer_assembled_txns_total")
	assert.Equal(t, metricFamilies[0].GetMetric()[0].GetCounter().GetValue(), float64(1))

	// Assembled transactions metric
	assert.Equal(t, metricFamilies[1].GetName(), "distributed_sequencer_dispatched_txns_total")
