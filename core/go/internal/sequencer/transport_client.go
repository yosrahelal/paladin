...
func (sMgr *sequencerManager) handleAssembleRequest(ctx context.Context, message *components.ReceivedMessage) {
	...
	if seq == nil {
		log.L(ctx).Debugf("ignoring assemble request for contract %s as sequencer is not loaded", contractAddress)
		return
	}
...
func (sMgr *sequencerManager) handleAssembleResponse(ctx context.Context, message *components.ReceivedMessage) {
	...
	seq := sMgr.GetSequencer(ctx, *contractAddress)
	if seq == nil {
		log.L(ctx).Debugf("ignoring assemble response for contract %s as sequencer is not loaded", contractAddress)
		return
	}
...
func (sMgr *sequencerManager) handleAssembleError(ctx context.Context, message *components.ReceivedMessage) {
	...
	seq := sMgr.GetSequencer(ctx, *contractAddress)
	if seq == nil {
		log.L(ctx).Debugf("ignoring assemble error for contract %s as sequencer is not loaded", contractAddress)
		return
	}
...
func (sMgr *sequencerManager) handlePreDispatchRequest(ctx context.Context, message *components.ReceivedMessage) {
	...
	seq := sMgr.GetSequencer(ctx, *contractAddress)
	if seq == nil {
		log.L(ctx).Debugf("ignoring pre dispatch request for contract %s as sequencer is not loaded", contractAddress)
		return
	}
...
func (sMgr *sequencerManager) handlePreDispatchResponse(ctx context.Context, message *components.ReceivedMessage) {
	...
	seq := sMgr.GetSequencer(ctx, *contractAddress)
	if seq == nil {
		log.L(ctx).Debugf("ignoring pre dispatch response for contract %s as sequencer is not loaded", contractAddress)
		return
	}
...
func (sMgr *sequencerManager) handleDispatchedEvent(ctx context.Context, message *components.ReceivedMessage) {
	...
	seq := sMgr.GetSequencer(ctx, *contractAddress)
	if seq == nil {
		log.L(ctx).Debugf("ignoring dispatched event for contract %s as sequencer is not loaded", contractAddress)
		return
	}
...
func (sMgr *sequencerManager) handleDelegationResponse(ctx context.Context, message *components.ReceivedMessage) {
	...
	if !delegationRequestAcknowledgment.Accepted {
		...
		seq := sMgr.GetSequencer(ctx, *contractAddress)
		if seq == nil {
			log.L(ctx).Debugf("ignoring delegation rejection for contract %s as sequencer is not loaded", contractAddress)
			return
		}
...
func (sMgr *sequencerManager) handleHandoverRequest(ctx context.Context, message *components.ReceivedMessage) {
	...
	seq := sMgr.GetSequencer(ctx, *contractAddress)
	if seq == nil {
		log.L(ctx).Debugf("ignoring handover request for contract %s as sequencer is not loaded", contractAddress)
		return
	}
...
func (sMgr *sequencerManager) handleEndorsementResponse(ctx context.Context, message *components.ReceivedMessage) {
	...
	seq := sMgr.GetSequencer(ctx, *contractAddress)
	if seq == nil {
		log.L(ctx).Debugf("ignoring endorsement response for contract %s as sequencer is not loaded", contractAddress)
		return
	}
...
func (sMgr *sequencerManager) handleNonceAssigned(ctx context.Context, message *components.ReceivedMessage) {
	...
	seq := sMgr.GetSequencer(ctx, *contractAddress)
	if seq == nil {
		log.L(ctx).Debugf("ignoring nonce assigned event for contract %s as sequencer is not loaded", contractAddress)
		return
	}
...
func (sMgr *sequencerManager) handleTransactionSubmitted(ctx context.Context, message *components.ReceivedMessage) {
	...
	seq := sMgr.GetSequencer(ctx, *contractAddress)
	if seq == nil {
		log.L(ctx).Debugf("ignoring transaction submitted event for contract %s as sequencer is not loaded", contractAddress)
		return
	}
...
func (sMgr *sequencerManager) handleTransactionConfirmed(ctx context.Context, message *components.ReceivedMessage) {
	...
	seq := sMgr.GetSequencer(ctx, *contractAddress)
	if seq == nil {
		log.L(ctx).Debugf("ignoring transaction confirmed event for contract %s as sequencer is not loaded", contractAddress)
		return
	}
...
func (sMgr *sequencerManager) handleNotActiveCoordinator(ctx context.Context, message *components.ReceivedMessage) {
	...
	seq := sMgr.GetSequencer(ctx, *contractAddress)
	if seq == nil {
		log.L(ctx).Debugf("ignoring not active coordinator event for contract %s as sequencer is not loaded", contractAddress)
		return
	}
...
func (sMgr *sequencerManager) handleTransactionUnknown(ctx context.Context, message *components.ReceivedMessage) {
	...
	seq := sMgr.GetSequencer(ctx, *contractAddress)
	if seq == nil {
		log.L(ctx).Debugf("ignoring transaction unknown event for contract %s as sequencer is not loaded", contractAddress)
		return
	}
