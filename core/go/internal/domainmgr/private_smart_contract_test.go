...
func TestFullTransactionRealDBOK(t *testing.T) {
	...
	err := psc.AssembleTransaction(dCtx, td.c.dbTX, tx)
	require.NoError(t, err)
	...
	err = psc.WritePotentialStates(dCtx, td.c.dbTX, tx)
	require.NoError(t, err)
	...
	// Lock all the states
	err = psc.LockStates(dCtx, td.c.dbTX, tx)
	require.NoError(t, err)
...
func TestIncompleteStages(t *testing.T) {
	...
	err = psc.LockStates(td.mdc, td.c.dbTX, tx)
...
func TestGetDomainReceiptAllAvailable(t *testing.T) {
	...
	resData, err := psc.GetDomainReceipt(td.ctx, td.c.dbTX, txID)
	require.NoError(t, err)
...
func TestGetDomainReceiptFail(t *testing.T) {
	...
	_, err := psc.GetDomainReceipt(td.ctx, td.c.dbTX, txID)
	assert.Regexp(t, "pop", err)
