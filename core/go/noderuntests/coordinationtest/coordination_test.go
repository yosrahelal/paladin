...
func TestAlternativeCoordinatorSelectedWhenPreferredUnavailable(t *testing.T) {
	// Test that when the preferred coordinator is unavailable the alternative node steps up to
	// coordinate transactions, and that once the preferred coordinator comes back online it
	// resumes as coordinator.
	//
	// A very large block range (math.MaxUint64) pins the entire test to a single coordinator
	// epoch, so the preferred coordinator cannot rotate mid-test due to a new block range
	// boundary. With that range, the effective block number used by SelectCoordinatorNode is
	// always 0 regardless of actual chain height, giving a deterministic preferred coordinator
	// that we can compute here before any transactions are submitted.
	//
	// The domain instance uses PrivacyGroupEndorsement with RequiredEndorsements=1 (partial
	// endorsement). Either node in the endorsement set can therefore satisfy the threshold on
	// its own, allowing the non-preferred node to dispatch without the preferred being available.
	ctx := t.Context()
	domainRegistryAddress := deployDomainRegistry(t, "alice")
	...
	bigBlockRange := uint64(math.MaxUint64)
	alice.OverrideSequencerConfig(&pldconf.SequencerConfig{BlockRange: &bigBlockRange})
	bob.OverrideSequencerConfig(&pldconf.SequencerConfig{BlockRange: &bigBlockRange})
	...
	// Deploy a domain instance with both nodes in the endorsement set and threshold=1.
	constructorParameters := &domains.ConstructorParameters{
		From:                 alice.GetIdentity(),
		Name:                 "FailoverToken",
		Symbol:               "FT",
	...
	// Determine the preferred coordinator using the same algorithm as production.
	// With blockRange=MaxUint64 the effective block number is always
	//   effectiveBlock = height - (height % MaxUint64) = 0
	// for any realistic chain height, so the preferred coordinator is the same regardless of the
	// actual block height. We therefore pass 0 directly rather than querying the chain.
	pool := seqcommon.DedupeSortedCoordinatorEndorserNodes([]string{alice.GetName(), bob.GetName()})
	preferred, _ := seqcommon.SelectCoordinatorNode(ctx, pool, 0, math.MaxUint64, 0)

	var preferredParty, nonPreferredParty testutils.Party
	switch preferred {
	case alice.GetName():
		preferredParty, nonPreferredParty = alice, bob
	case bob.GetName():
		preferredParty, nonPreferredParty = bob, alice
	default:
		t.Fatalf("unexpected preferred coordinator: %s", preferred)
	}

	// nonPreferred stays running for the whole test.
	t.Cleanup(func() {
		stopNode(t, nonPreferredParty)
	})

	// Stop the preferred coordinator. The non-preferred node should now act as coordinator.
	stopNode(t, preferredParty)

	// Submit tx1 from the non-preferred node. With threshold=1 it can endorse its own transaction
	// and dispatch to the chain without the preferred coordinator being available.
	tx1 := nonPreferredParty.GetClient().ForABI(ctx, *domains.SimpleTokenTransferABI()).
		Private().
		Domain("domain1").
		IdempotencyKey("tx1-failover-" + uuid.New().String()).
		From(nonPreferredParty.GetIdentity()).
		To(contractAddress).
		Function("transfer").
		Inputs(pldtypes.RawJSON(`{
			"from": "",
			"to": "` + nonPreferredParty.GetIdentityLocator() + `",
			"amount": "1000000000000000000"
		}`)).
		Send().Wait(transactionLatencyThreshold(t))
	require.NoError(t, tx1.Error())

	// Verify the non-preferred node coordinated tx1.
	tx1Full, err := nonPreferredParty.GetClient().PTX().GetTransactionFull(ctx, tx1.ID())
	require.NoError(t, err)
	require.Len(t, tx1Full.SequencerActivity, 1)
	assert.Equal(t, string(pldapi.SequencerActivityType_Dispatch), tx1Full.SequencerActivity[0].ActivityType)
	assert.Equal(t, nonPreferredParty.GetName(), tx1Full.SequencerActivity[0].SequencingNode,
		"expected non-preferred node to coordinate tx1 while preferred was stopped")

	// Restart the preferred coordinator. Restarting alone does not load the sequencer for the
	// contract; that happens lazily when an incoming endorsement request arrives from a subsequent
	// transaction submitted by the non-preferred node.
	startNode(t, preferredParty, domainConfig)
	t.Cleanup(func() {
		stopNode(t, preferredParty)
	})

	// Submit tx2 from the non-preferred node. The endorsement request sent to the preferred node
	// causes its sequencer to load for this contract, which may trigger the non-preferred node to
	// yield coordination back to the preferred.
	tx2 := nonPreferredParty.GetClient().ForABI(ctx, *domains.SimpleTokenTransferABI()).
		Private().
		Domain("domain1").
		IdempotencyKey("tx2-failover-" + uuid.New().String()).
		From(nonPreferredParty.GetIdentity()).
		To(contractAddress).
		Function("transfer").
		Inputs(pldtypes.RawJSON(`{
	...
	tx2Full, err := nonPreferredParty.GetClient().PTX().GetTransactionFull(ctx, tx2.ID())
	require.NoError(t, err)
	require.NotEmpty(t, tx2Full.SequencerActivity)

	// tx2 may carry more than one activity entry: the non-preferred node could have started
	// coordinating before the preferred node's sequencer loaded, causing a handover mid-flight.
	// We only need to know whether the preferred node dispatched at any point.
	preferredAlreadyCoordinating := false
	for _, a := range tx2Full.SequencerActivity {
		if a.ActivityType == string(pldapi.SequencerActivityType_Dispatch) && a.SequencingNode == preferredParty.GetName() {
			preferredAlreadyCoordinating = true
			break
		}
	}
	if preferredAlreadyCoordinating {
		// Preferred coordinator has already taken over — test is complete.
		return
	}

	// The non-preferred was still acting as coordinator long enough to submit tx2 (it started sequencing tx2
	// before the preferred node's sequencer was fully loaded). Submit tx3; by now the preferred node's
	// sequencer is loaded and should assert itself as coordinator.
	tx3Sent := nonPreferredParty.GetClient().ForABI(ctx, *domains.SimpleTokenTransferABI()).
		Private().
		Domain("domain1").
		IdempotencyKey("tx3-failover-" + uuid.New().String()).
		From(nonPreferredParty.GetIdentity()).
		To(contractAddress).
		Function("transfer").
		Inputs(pldtypes.RawJSON(`{
		...
		Send()
	tx3 := tx3Sent.Wait(transactionLatencyThreshold(t))

	if tx3.Error() != nil {
		// During coordinator handover both the outgoing and incoming coordinator may each assemble
		// and dispatch the same transaction. The second on-chain call reverts with
		// SimpleTokenDuplicateTransaction(bytes32) — ABI error selector 0xa810cfac.
		// This is an expected race; confirm it is that known revert, then verify that
		// exactly two dispatches occurred and the preferred coordinator was responsible for one.
		require.True(t, strings.Contains(tx3.Error().Error(), "a810cfac"),
			"unexpected revert during coordinator handover: %v", tx3.Error())

		tx3Full, err := nonPreferredParty.GetClient().PTX().GetTransactionFull(ctx, tx3.ID())
		require.NoError(t, err)
		require.Len(t, tx3Full.SequencerActivity, 2,
			"expected two dispatch entries for tx3 during coordinator handover")

		preferredDispatchedTx3 := false
		for _, a := range tx3Full.SequencerActivity {
			if a.ActivityType == string(pldapi.SequencerActivityType_Dispatch) && a.SequencingNode == preferredParty.GetName() {
				preferredDispatchedTx3 = true
				break
			}
		}
		assert.True(t, preferredDispatchedTx3,
			"expected preferred coordinator to dispatch tx3 during handover")
		return
	}

	// Clean success path: no duplicate dispatch — preferred coordinator won outright.
	tx3Full, err := nonPreferredParty.GetClient().PTX().GetTransactionFull(ctx, tx3.ID())
	require.NoError(t, err)
	require.NotEmpty(t, tx3Full.SequencerActivity)
	...
	assert.True(t, preferredDispatchedTx3,
		"expected preferred coordinator to resume coordination after coming back online")
}
