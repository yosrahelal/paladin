...
func TestRCPMethodInvalidValue(t *testing.T) {
	...
	assert.Regexp(t, "PD010804", errResponse.Error.Message)
...
func TestRCPMethodWrongParamCount(t *testing.T) {
	...
	assert.False(t, res.IsSuccess())
	assert.Equal(t, int64(rpcbackend.RPCCodeInvalidRequest), errResponse.Error.Code)
	assert.Regexp(t, "PD010803", errResponse.Error.Message)
...
func TestRCPMethodBadResult(t *testing.T) {
	...
	assert.Regexp(t, "PD010805", errResponse.Error.Message)
