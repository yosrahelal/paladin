...
var (
	...
	MsgSigningKeyListingNotSupported            = ffe("PD011414", "Listing keys in the key store is not supported by this signing module")
	MsgSigningStaticKeyInvalid                  = ffe("PD011415", "Statically configured key with handle %s is invalid")
	MsgSigningStaticBadEncoding                 = ffe("PD011416", "Statically configured key with handle %s has invalid encoding (must be one of 'none', 'hex', 'base64') '%s'")
	MsgSigningKeyCannotBeResolved               = ffe("PD011417", "No key exists that matches the request")
)
