...
func TestCheckGeneratedMarkdownPages(t *testing.T) {
	...
	for pageName, markdown := range markdownMap {
		...
		assert.Equal(t, expectedPageHash.Sum(nil), actualPageHash.Sum(nil), "The type reference docs generated by the code did not match the docs files in git for page: '%s.md' Did you forget to run `gradle generateDocs`?", pageName)
	}
}
