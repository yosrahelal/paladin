...
export const Registry: React.FC<Props> = ({ registryName }) => {
  ...
  const [registryEntries, setRegistryEntries] = useState<IRegistryEntry[]>([]);
  ...
  useEffect(() => {
    let requestPayload = {
      ...
      params: [registryName, { limit: constants.REGISTRY_ENTRIES_QUERY_LIMIT }, 'any']
    };
    fetch('/json-rpc', {
      method: 'post',
      headers: {
        ...
        <Box key={registryEntry.id} sx={{
          ...
          <Grid2 container direction="column" spacing={2}>
            <Grid2 container justifyContent="space-evenly">
              <Grid2>
                <Typography align="center" variant="h6" color="textPrimary">{registryEntry.name}</Typography>
                <Typography align="center" variant="body2" color="textSecondary">{t('name')}</Typography>
              </Grid2>
              <Grid2>
                <Typography align="center" variant="h6" color="textPrimary">{registryEntry.registry}</Typography>
                <Typography align="center" variant="body2" color="textSecondary">{t('registry')}</Typography>
              </Grid2>
              <Grid2>
              ...
              <Grid2>
                <Hash title={t('owner')} hash={registryEntry.properties.$owner} />
                <Typography align="center" variant="body2" color="textSecondary">{t('owner')}</Typography>
              </Grid2>
              <Grid2 sx={{ textAlign: 'center' }} alignContent="center">
                {registryEntry.active ? <CheckCircleOutlineIcon color="primary" /> : <ErrorOutlineIcon color="error" />}
                <Typography align="center" variant="body2" color="textSecondary">{t('active')}</Typography>
              </Grid2>
            </Grid2>
