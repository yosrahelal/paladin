...
func TestPrepareUnlock(t *testing.T) {
	...
	expectedCancelHash, err := n.unlockHashFromIDs_V1(ctx, ethtypes.MustNewAddress(contractAddress), lockID, lockInfo.SpendTxId.HexString(), endorsableStateIDs(readStates, false), endorsableStateIDs(infoStates[2:3], false), unlockTxData)
	require.NoError(t, err)
