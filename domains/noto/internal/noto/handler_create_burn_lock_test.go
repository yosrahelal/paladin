...
func TestCreateBurnLock(t *testing.T) {
	...
	expectedCancelHash, err := n.unlockHashFromIDs_V1(ctx, ethtypes.MustNewAddress(contractAddress), lockID, lockInfo.SpendTxId.HexString(), endorsableStateIDs(outputStates[1:2], false), endorsableStateIDs(infoStates[1:2], false), unlockTxData)
	require.NoError(t, err)
