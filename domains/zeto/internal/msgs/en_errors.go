...
var (
	...
	MsgErrorVerifierTypeMismatch          = pde("PD210089", "'%s' does not match supported verifierType '%s'")
	...
	MsgErrorLockDelegateNotFound          = pde("PD210109", "lock delegate not found by ID in the local wallet: %s")
	MsgErrorMissingLockInputs             = pde("PD210110", "locked inputs are required")
	MsgErrorQueryLockedInputs             = pde("PD210111", "Failed to query the state store for locked inputs. Expected: %d. Found: %d")
	MsgErrorInputNotLocked                = pde("PD210112", "Input %s is not locked")
	MsgErrorInsufficientInputAmount       = pde("PD210113", "Insufficient input amount (total=%s) for the transfers (total=%s)")
