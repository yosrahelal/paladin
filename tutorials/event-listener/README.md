...
- **Extract contracts**
  ...
  - Extract the `abis.tar.gz` file that was downloaded in the [setup steps](../README.md) and copy the `abis` directory to `src/`. (full path should be: `paladin/tutorials/event-listener/src/abis`)

- **Run the example**

  ```shell
  cd <paladin-root>/tutorials/event-listener
  npm install
  npm run start
  ```
